...
export const fetchGallery = id => dispatch =>
  GalleryAPIUtil.fetchGallery(id).then(gallery =>
    dispatch(receiveGallery(gallery))
  );

export const fetchAllGalleries = () => dispatch =>
  GalleryAPIUtil.fetchGalleries().then(
...
export const createGallery = gallery => dispatch =>
  GalleryAPIUtil.createGallery(gallery).then(
    gallery => dispatch(receiveGallery(gallery))
    // err => dispatch(receiveGalleryErrors(err.responseJSON))
  );

export const updateGallery = gallery => dispatch =>
  GalleryAPIUtil.updateGallery(gallery).then(
    gallery => dispatch(receiveGallery(gallery)),
    err => dispatch(receiveGalleryErrors(err.responseJSON))
  );

export const deleteGallery = galleryId => dispatch =>
  GalleryAPIUtil.deleteGallery(galleryId).then(
    gallery => dispatch(removeGallery(galleryId)),
    err => dispatch(receiveGalleryErrors(err.responseJSON))
  );

export const createGalleryItem = galleryItem => dispatch =>
  GalleryAPIUtil.createGalleryItem(galleryItem).then(
    () => dispatch(fetchAllGalleries()),
    err => dispatch(receiveGalleryErrors(err.responseJSON))
  );

export const deleteGalleryItem = galleryItemId => dispatch =>
  GalleryAPIUtil.deleteGalleryItem(galleryItemId).then(() =>
    dispatch(fetchAllGalleries())
  );
